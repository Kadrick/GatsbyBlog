...
slug: aws/lambda/vpc-setting
---

## 문제 상황

AWS Lambda 함수를 작성하던 중, AWS RDS와의 연결과 API 호출이 필요한 상황이었다.
RDS 연결을 위해서 Lambda에 이미 VPC를 설정해놓은 상태였는데, 이후 API 호출이 실패하기 시작했다.
...
Lambda를 실행할 때는 NAT Gateway를 통해서 할당된 사설 IP를 인터넷 통신을 위한 공인 IP로 바꿔 주어야 한다는 것이었다.
